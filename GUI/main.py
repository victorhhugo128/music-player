...
class Layout(tk.Tk):
    def __init__(self):
        ...
        # menu de opções
        self.menu_de_opcoes = tk.Menu(self)

        # menu file
        fileMenu = tk.Menu(self.menu_de_opcoes, tearoff=0)
        fileMenu.add_command(label="Adicionar Diretório", command=search_directory)
        fileMenu.add_command(label="Open file")
        fileMenu.add_command(label="Save")
        self.menu_de_opcoes.add_cascade(label="File", menu=fileMenu)

        # outras opções, mas sem o cascade
        self.menu_de_opcoes.add_command(label="Edit")
        self.menu_de_opcoes.add_command(label="View")
        self.menu_de_opcoes.add_command(label="Navigate")

        self.config(menu=self.menu_de_opcoes)
    ...
    def update_cover(capa: tk.Label):
        """
        Atualiza a capa do álbum de acordo com a música que está tocando no momento.
        :param capa: imagem da capa do álbum.
        :return: None.
        """
    ...
    def next_button(self):
        """
        Faz as devidas configurações quando o botão de próximo é acionado.
        :return: None.
        """
    ...
    def previous_button(self):
        """
        Faz as devidas configurações quando o botão de anterior é acionado.
        :return: None.
        """
        previous_song()
        sleep(0.1)
    ...
    def progress_bar(self):
        ...
        if self.control:
            if get_current_play_time() != 0:
                self.music_bar.configure(value=get_current_time()/get_current_play_time() + get_start_time()/get_current_play_time())
    ...
    def repeat_config(self):
        """
        Faz a manutenção da imagem de repetir que aparece na interface.
        :return: None.
        """
        repeat_songs()
    ...
    def random_config(self):
        """
        Faz a manutenção da imagem de aleatório que aparece na interface.
        :return: None.
        """
        randomize_songs()
    ...
    def set_timestamp(self, event):
        """
        Cuida da manutenção da barra de progressão interativa.
        :param event: evento de interação com a barra.
        :return:
        """
